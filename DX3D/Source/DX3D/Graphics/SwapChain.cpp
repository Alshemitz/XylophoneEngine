	...
	GraphicsResource(gDesc)
{
	...
	DX3DGraphicsLogErrorThrowOnFail(m_factory.CreateSwapChain(&m_device, &dxgiDesc, &m_swapChain),
...
{
    DX3DGraphicsLogErrorThrowOnFail(m_swapChain->Present(vsync, 0),
		"Present failed");
}
...
void dx3d::SwapChain::reloadBuffers()
{
    ...
    DX3DGraphicsLogErrorThrowOnFail(m_swapChain->GetBuffer(0, 
		IID_PPV_ARGS(&m_backBuffer)),
        "GetBuffer failed");

	DX3DGraphicsLogErrorThrowOnFail(m_device.CreateRenderTargetView(
		m_backBuffer.Get(),
		nullptr,
		&m_backBufferRTV),
